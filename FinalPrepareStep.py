...
def read_csv_tenure(filename, outputPath):
    ...
    for i, r in df.iterrows():
        if pd.isnull(r["FQnum"]):
            ...
            df1.at[i, "FQnum"] = qnum
            # df1.at[i, "Wikidata Link"] = 'https://www.wikidata.org/wiki/' + qnum
...
def read_csv_emeritus(filename, outputPath):
    df = pd.read_csv(filename)
    df1 = df.copy()
    for i, r in df.iterrows():
        if pd.isnull(r["FQnum"]):
...
if __name__ == "__main__":
    ...
    typeOfFaculty = str(input("Enter \'t\' for Tenure or \'e\' for Emeritus: "))
    folder = str(input("\nEnter folder name: ")) + '/'
    filename = str(input("\nEnter file name(without extension): "))
    dest = str(input("\nEnter output file name (without extension): "))

    source = '/Users/sprasad/Documents/SDSU/CAL/' + folder + filename + '.csv'
    destination = '/Users/sprasad/Documents/SDSU/CAL/' + folder + 'final/' + dest + '.csv'
    ...
    if typeOfFaculty == 't':
        ...
        print("FAIL: Please enter proper code for type of faculty.")

    print("SUCCESS: File saved: ", destination)
